...
def get_travelers_rows(n, default_dict):
    ...
    phone_row = [sg.Text('Contact WhatsApp num', size=(W2, 1)),
...
def get_flight_input_frame(i, default_dict):
    ...
    details_col = [[
        sg.Frame("Details", [
            ...
            [sg.Text("Compartment", size=(W2,1)), sg.Drop(COMPARTEMENTS, key=f"compartment{i}", size=(W2 - 1, 1), default_value=compartement_default)]
        ])
    ]]
    columns = [sg.Column(depart_col), sg.Column(dest_col), sg.Column(details_col)]

    # add an amadeus parser above the columns
    amadeus_row = [sg.Text("Amadeus code:", size=(W2,1)),
                   sg.Input(key=f"amadeus_code{i}", size=(W2*6,1), default_text=default_dict[f"amadeus_code{i}"]),
...
def get_price_and_restriction_rows(default_dict):
    rows = [[
        sg.Frame('Price', [
            [sg.Text("Price (adult)", size=(W2, 1)), sg.Input(size=(W3, 1), key="price_adult", default_text=default_dict["price_adult"])],
            [sg.Text("Price (child)", size=(W2, 1)), sg.Input(size=(W3, 1), key="price_child", default_text=default_dict["price_child"])],
            [sg.Text("fee percentage", size=(W2, 1)),
             sg.Input(size=(W3, 1), key="price_fee", default_text=default_dict["price_fee"])]
        ]),

        sg.Frame('Restrictions', [
            [sg.Text("Change fee", size=(W2, 1)),
             sg.Input(size=(W3, 1), key="change_fee", default_text=default_dict["change_fee"])],
            [sg.Text("Cancel fee", size=(W2, 1)),
             sg.Input(size=(W3, 1), key="cancel_fee", default_text=default_dict["cancel_fee"])],
            [sg.Text("No show fee", size=(W2, 1)),
             sg.Input(size=(W3, 1), key="no_show_fee", default_text=default_dict["no_show_fee"])],
        ]
                 )
    ]]
    return rows
...
def get_total_layout(n_travelers, n_flights, default_values, preview_msg):
    ...
    layout = [[sg.Column(layout), sg.Column([[sg.Multiline(size=(50, 35 + (n_flights-1)*12), default_text=preview_msg, key='output_window')]])]]
